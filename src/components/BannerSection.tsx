    ...
    <section className="relative bg-linear-to-b from-black via-gray-950 to-black py-16 md:py-20 border-b border-t border-gray-700/80">
      <div className="mx-auto max-w-7xl px-4 md:px-10 lg:px-16">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <ul className="grid grid-cols-2 lg:grid-cols-4 gap-x-6 md:gap-x-12 gap-y-6 md:gap-y-10">
            {bullets.map((item, idx) => (
              <li key={idx} className="flex items-start gap-4">
                <span className="mt-3 h-2 w-2 rounded-full bg-purple-400 shadow-[0_0_10px_rgba(168,85,247,0.6)]" />

                <div className="flex flex-col">
                  <h3 className="text-xl md:text-2xl lg:text-3xl xl:text-4xl font-black text-purple-100 leading-tight">
                    {item.title}
                  </h3>
